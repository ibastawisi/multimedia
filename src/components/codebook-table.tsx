...
export default function CodebookTable({ codebook, frequencies, currentStep }: CodebookTableProps) {
        ...
        {Object.keys(codebook).length === 0 ? (
          ...
          <>
                ...
                {Object.keys(codebook)
                  ...
                  .map((char) => (
                    <TableRow key={char}>
                      ...
                      <TableCell className="font-mono">{codebook[char]}</TableCell>
                      <TableCell>{codebook[char].length}</TableCell>
                      <TableCell>{codebook[char].length * frequencies[char]}</TableCell>
                    </TableRow>
                  ))}
              </TableBody>
            </Table>
            ...
            <div className="mt-4 p-3 bg-gray-50 rounded-md">
              <p className="text-sm">
                <span className="font-medium">Average Code Length:</span> {avgCodeLength.toFixed(2)} bits per character
              </p>
              <p className="text-sm mt-2">
                <span className="font-medium">Note:</span> The Shannon-Fano algorithm assigns shorter codes to more
                frequent characters, which results in efficient compression.
              </p>
            </div>
          </>
        )}
