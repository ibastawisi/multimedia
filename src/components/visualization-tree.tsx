...
export default function VisualizationTree({
  treeData,
  currentPartition,
  currentStep,
    ...
    const updateDimensions = () => {
      if (svgRef.current) {
        ...
        setDimensions({
          ...
          height: Math.max(
            500,
            treeData ? calculateTreeHeight(treeData) * 100 : 300
          ),
        });
      }
    };
  ...
  const renderTree = (
    ...
    const isHighlighted =
      currentPartition &&
      currentPartition.chars.join(",") === node.name.split(", ").join(",");

    // Calculate positions for children
    const childrenElements: React.JSX.Element[] = [];
    const edgeElements: React.JSX.Element[] = [];
    ...
    if (node.children.length > 0) {
      ...
      for (let i = 0; i < node.children.length; i++) {
        ...
        edgeElements.push(
          <g key={`edge-${node.id}-${i}-${childX}-${childY}`}>
            <line
              x1={x}
              y1={y}
              x2={childX}
        ...
        const childElements = renderTree(
          node.children[i],
          childX,
          childY,
          childWidth,
    ...
    const nodeElement = (
      <g key={`node-${node.id}-${x}-${y}`}>
        ...
        <text
          ...
          y={y - 5}
          textAnchor="middle"
          fontSize={node.name.length > 5 ? 10 : 12}
          fill={textColor}
        >
          {node.name}
        </text>
        ...
        {isHighlighted &&
          node.leftSum !== undefined &&
          node.rightSum !== undefined && (
            <>
              <text
                x={x - 15}
                y={y + nodeRadius + 45}
  ...
  return (
    <div className="overflow-x-auto">
      ...
      >
        <g transform={`translate(${dimensions.width / 2}, 50)`}>
          {renderTree(treeData, 0, 0, dimensions.width * 0.8)}
        </g>
      </svg>
